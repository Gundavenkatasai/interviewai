...
export default function HistoryPage() {
  ...
  const handleDelete = async (id: string, e: React.MouseEvent) => {
    e.preventDefault();
    e.stopPropagation();
    if (!confirm("Are you sure you want to delete this interview record permanently?")) return;
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-400 mt-1">
            Review past mock interview transcripts, feedback scores, and diagnostic reports.
          </p>
        </div>

        <Link
          href="/setup"
          className="px-5 py-2.5 rounded-xl text-xs font-semibold text-white bg-indigo-600 hover:bg-indigo-500 shadow-md shadow-indigo-600/20 transition-colors"
        >
          Start New Practice
        </Link>
      </div>
      ...
      {interviews.length > 0 ? (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          {interviews.map((sess) => {
            ...
            return (
              <div
                key={sess.id}
                className="group relative p-6 rounded-2xl bg-slate-900/50 border border-slate-800/80 hover:border-slate-700 backdrop-blur-md transition-all flex flex-col justify-between space-y-4"
              >
                <div>
                  <div className="flex items-start justify-between gap-3 mb-2">
                    <div>
                      ...
                      {sess.company && (
                        <p className="text-xs text-slate-400 mt-0.5 font-medium">@ {sess.company}</p>
                      )}
                    </div>
                ...
                <div className="flex items-center justify-between pt-3 border-t border-slate-800/80">
                  <div className="text-xs">
                    {sess.score ? (
                      <span className="font-bold text-white">
                        Score: <strong className="text-indigo-400">{sess.score.overall_score}</strong> / 10
                      </span>
                    ) : (
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <Link
                      href={linkHref}
                      className="px-3.5 py-1.5 rounded-lg text-xs font-semibold text-white bg-slate-800 hover:bg-slate-700 flex items-center gap-1.5 transition-colors"
                    >
        ...
        <div className="p-12 rounded-3xl bg-slate-900/40 border border-slate-800 text-center space-y-4">
          ...
          <p className="text-xs text-slate-400 max-w-sm mx-auto">
            Ready to test your technical depth? Launch a session to start tracking your interview mastery.
          </p>
          <Link
            href="/setup"
            className="inline-block px-5 py-2.5 rounded-xl bg-indigo-600 text-white text-xs font-semibold"
          >
